...
async function addReviewService(user_id: string, driver_id: string, order_id: string, stars: number, comment?: string) {
    if (!user_id) throw AppError("User ID is required", 400);
    if (!driver_id) throw AppError("Driver ID is required", 400);
    if (!order_id) throw AppError("Order ID is required", 400); // changed
    if (stars > 5 || stars <= 0) throw AppError("Input stars range start from 1-5", 400);
    const review = new driverReview({ user_id, driver_id, order_id, stars, comment }); // changed
    await review.save();
}
...
async function getReviewsService(page: number = 1, limit: number = 10, search?: string) {
    ...
    if (search) {
        ...
        { order_id: { $regex: search, $options: "i" } } // changed
        ];
    }
