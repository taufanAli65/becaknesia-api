...
import { addReviewService, getReviewsService, getReviewService, updateReviewService, deleteReviewService } from "../services/tourReview";
import { sendSuccess, sendFail } from "../utils/senResponse";
import { validate } from "../utils/validate";
import { addReviewSchema, needReviewIDSchema, updateReviewSchema } from "../validators/tourReview";
...
export const addReview = async(req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const {tour_id, stars, comment} = validate(addReviewSchema, req.body);
        const tour = Tour.findById(tour_id);
        if(!tour) throw AppError("There is no tour with such ID", 404);
        await addReviewService(user_id, tour_id, stars, comment);
    } catch (error) {
